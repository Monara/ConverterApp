    ...
    private TextView conversionResult;
            ...
            buttonConvert.setOnClickListener(new View.OnClickListener() {
                public void onClick(View v) {    //when conversion button is clicked
                    ...
                    if (inputValue.getText().toString().trim().length() == 0) {  // if input box is empty
                        Toast prompt = Toast.makeText(Temperature.this, "Please insert a number", Toast.LENGTH_LONG);
                        ...
                        double temperatureConverterResult = Converter.convertTemperatureUnits(finalInputValue, fromUnitName, toUnitName);    //get a conversion result
                        conversionResult.setText(Double.toString(temperatureConverterResult));   //set the conversion result into the output box
                    }
                }
            });
        ...
        public void addToFavorite (View v){
            ...
            if (inputValue.getText().toString().length() == 0) {  // if input box is empty
                ...
                double temperatureConverterResult = Converter.convertTemperatureUnits(finalInputValue, fromUnitName, toUnitName);

                FavouriteItem favouriteItem = new FavouriteItem(fromUnitName, toUnitName, finalInputValue, temperatureConverterResult);
                ...
                Toast prompt = Toast.makeText(Temperature.this, "Added to favourites", Toast.LENGTH_SHORT);
                prompt.show();
            }
